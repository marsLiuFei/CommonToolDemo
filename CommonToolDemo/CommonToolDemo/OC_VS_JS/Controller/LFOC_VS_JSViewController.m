...
@interface LFOC_VS_JSViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic, strong)UITableView *tableView;
@property(nonatomic, copy)NSArray *titles;
@end

@implementation LFOC_VS_JSViewController
-(NSArray *)titles{
    if (!_titles) {
        _titles = @[@"WKWebView中OC与JS交互",@"UIWebView中OC与JS交互"];
    }
    return _titles;
}
- (void)viewDidLoad {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==0) {
        ...
        LFUIWebViewController *vc = [LFUIWebViewController new];
        [self.navigationController pushViewController:vc animated:YES];
    }
}
